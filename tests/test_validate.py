"""Schema validation of the com.google.cloud.data.agent-plugins source bucket."""
...
class TestValidatePlugin:
    ...
    def test_error_messages_are_namespaced_for_context(self, make_plugin, tmp_path):
        """Errors are prefixed with com.google.cloud.data.agent-plugins so authors can locate them."""
        # Arrange
        ...
        assert all(message.startswith("com.google.cloud.data.agent-plugins") for message in result.errors)
